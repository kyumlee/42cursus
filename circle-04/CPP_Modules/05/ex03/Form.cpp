...
void				Form::executable(const Bureaucrat &br) const {
	if (!_signed)						throw IsNotSignedException();	
	if (br.getGrade() > _executeGrade)	throw GradeTooLowException();
}
...
const char			*Form::GradeTooLowException::what(void) const throw() {
	return ("Grade is too low! Lowest possible grade is 150. (or should be high enough to sign/execute a form)");
}

const char			*Form::IsNotSignedException::what(void) const throw() {
	return ("Form is not signed. Form must be signed before being executed.");
}
