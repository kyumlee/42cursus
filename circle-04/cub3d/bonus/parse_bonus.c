...
/*   Updated: 2021/05/28 19:26:28 by kyumlee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_duplicates(t_check_id *check, int fd)
{
	if (check->f > 1 || check->c > 1
		|| check->no > 1 || check->so > 1 || check->we > 1
		|| check->ea > 1 || check->s > 1)
	{
		close(fd);
		ft_error("Duplicate ID is found");
	}
	if (check->f == 0 || check->c == 0
		|| check->no == 0 || check->so == 0 || check->we == 0
		|| check->ea == 0 || check->s == 0)
	{
		close(fd);
		ft_error("Missing ID (NO, SO, WE, EA, S, F, C)");
	}
}
...
int		parse(t_all *all, char *s)
{
	...
	{
		if (!check_start(line))
			ft_error("Wrong ID / map should start with a wall (1)");
		parse_line(all, line, &count, &check);
	}
	check_duplicates(&check, fd);
	parse_map(&all->map);
	fill_map(&all->map);
	count_sprites(all, &all->map);
